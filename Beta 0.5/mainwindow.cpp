...
void MainWindow::on_listClouds_itemSelectionChanged()
{
    ...
    for(auto &i : selected)
    {
        ...
        {
            visualizer.selectCloudXYZI(i);
        }
        else if(tab->item(i, 2)->text() == "RGB")
        {
            visualizer.selectCloudXYZRGB(i);
        }
    }
    std::cout << std::endl << "DESELECTED: ";
    for(auto &i : deselected)
    {
        ...
        {
            visualizer.unselectCloudXYZI(i, ui->schemeXYZI->currentIndex());
        }
        else if(tab->item(i, 2)->text() == "RGB")
        {
            visualizer.unselectCloudXYZRGB(i, ui->schemeRGB->currentIndex());
        }
    }
    std::cout << std::endl;
