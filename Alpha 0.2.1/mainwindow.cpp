...
void MainWindow::updateClouds()
{
    ...
    for(QPushButton *del : ui->cloudsWidget->findChildren<QPushButton*>())
    {
    ...
    if(!cloudsXYZ.clouds.empty())
    {
        ...
        for(int i = 0; i < cloudsXYZ.clouds.size(); i++)
        {
            ...
            QPushButton *del = new QPushButton(ui->cloudsWidget);

            del->setText("-");
            del->setSizePolicy(QSizePolicy::Maximum, QSizePolicy::Fixed);
