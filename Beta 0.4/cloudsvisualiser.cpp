    ...
    for(int i = 0; i < cloudsXYZRGB->clouds.size(); i++)
    {
        if(dataXYZRGB[i].scheme != scheme)
        {
            ...
            for(int j = 0; j < dataXYZRGB[i].size; j++)
            {
                ...
                if(scheme == 3)
                {
                    r = cloudsXYZRGB->clouds[i].first[i].r();
                    g = cloudsXYZRGB->clouds[i].first[i].g();
                    b = cloudsXYZRGB->clouds[i].first[i].b();
                }
                else{
                    r = sin(a) * 192 + 128;
                    g = sin(a - 2 * M_PI / 3.0) * 192 + 128;
...
void cloudsVisualiser::selectCloud(int index)
{

}

void cloudsVisualiser::selectCloud(pointCloud<pointI> &cloud)
{

}

void cloudsVisualiser::selectCloud(pointCloud<pointRGB> &cloud)
{
